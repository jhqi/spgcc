...
#pixel cnn encoding
cnn_encoding = np.load('/sd1/qjh/qjh_code/98_try/DataArray/PU_60_standard_no6.npy')

#sp_map
sp_map = read_mat('/sd1/qjh/qjh_code/spgcc/sp_seg_map/PU_sp_map_2200.mat').astype(np.int32)
...
#superpixel adj
adj = np.load('/sd1/qjh/qjh_code/spgcc/sp_adj/PU_sp_adj_2200.npy')
adj = process_adj(adj)
adj = adj.to(device)
...
for seed in range(10):
    ...
    #model
    model = SPGCC(nfeat=cnn_encoding.shape[1], nhid=1024, nout=512, n_sp=n_sp, cluster_num=cluster_num, dropout=dropout, threshold=threshold, temperature=temperature, alpha=alpha, device=device).to(device)
    ...
    # init metric
    oa, aa, kappa, nmi, ari, f1, precision, recall, purity, predict_labels, dis = my_clustering_gpu(sp_feat, label_gt, cluster_num, labeled_p_in_sp)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch<=20:
            ...
            loss.backward()
            optimizer.step()
            print(f'epoch:{epoch}, loss:{loss.item()}, align_loss:{align_loss.item()}, contra_loss:{contra_loss.item()}')
        ...
        if epoch % 5 == 0:
            ...
            oa, aa, kappa, nmi, ari, f1, precision, recall, purity, predict_labels, dis = my_clustering_gpu(hidden_emb, label_gt, cluster_num, labeled_p_in_sp)
            ...
            if metric >= best_metric:
                ...
                no_better += 5
            print(f'epoch:{epoch}, oa:{oa}, aa:{aa}, kappa:{kappa}, nmi:{nmi}, ari:{ari}, f1:{f1}, precision:{precision}, recall:{recall}, purity:{purity}, best oa:{best_oa}, best_aa:{best_aa}, best kappa:{best_kappa}, best nmi:{best_nmi}, best_ari:{best_ari}, best_f1:{best_f1}, best_precision:{best_precision}, best_recall:{best_recall}, best_purity:{best_purity}')
