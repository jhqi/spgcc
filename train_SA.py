...
# pixel cnn encoding
cnn_encoding = np.load("pretrained_emb/SA_pretrained_emb.npy")

# prepare label
label_gt, labeled_p_in_sp = load_label(dataset_name, n_sp=n_sp)

# sp_map
sp_map = read_mat("sp_seg_map/SA_sp_map_2700.mat").astype(np.int32)
...
# superpixel adj
adj = np.load("sp_adj/SA_sp_adj_2700.npy")
adj = process_adj(adj)
adj = adj.to(device)
